...
namespace Farm
{
    public partial class fminc : Form
    {
        ...
        private void fminc_Load(object sender, EventArgs e)
        {
            loaddata();
        }
        ...
        {
            if (string.IsNullOrWhiteSpace(saltb.Text) || string.IsNullOrWhiteSpace(othertb.Text) || datepick.Value == null)
            {
                MessageBox.Show("Fill Out All the Fields!!!");
                return;
            }

            int sales = Convert.ToInt32(saltb.Text);
            int other = Convert.ToInt32(othertb.Text);
            DateTime incomedate = datepick.Value.Date;
            ...
            {
                con.Open();
                string sql = "INSERT INTO IncTable (Sales, Other, Date) VALUES (@s, @o, @date)";
        ...
        private void calandsave()
        {
            ...
            try
            {
                ...
                string totsql = "SELECT SUM(Sales + Other) FROM IncTable WHERE CAST (Date AS DATE) = @date";
                ...
                string insertsql = "INSERT INTO TransTable (Inc, Date) VALUES (@totalincome, @date)";

                SqlCommand cmdinsert = new SqlCommand(insertsql, con);
                cmdinsert.Parameters.AddWithValue("@totalincome", income);
        ...
        private void loaddata()
        {
            ...
            {
                con.Open();
                string sql = "SELECT * FROM IncTable";

                SqlDataAdapter da = new SqlDataAdapter(sql, con);
                DataTable dt = new DataTable();
                da.Fill(dt);

                incdgv.DataSource = dt;

                incdgv.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
